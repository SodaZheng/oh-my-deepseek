...
function printCreateResult(result, asJson) {
  ...
  if (result.platform === "darwin") {
    ...
    if (result.loadingRenderer === "chrome-html") process.stdout.write("启动画面：由 Windows Chrome 渲染三平台统一的小鲸鱼 loading\n");
    if (result.usesLoadingScreen) process.stdout.write("页面交接：DSH 完整就绪后在同一 Chrome 窗口淡出 loading\n");
    ...
    process.stdout.write("Windows 窗口门：小鲸鱼 loading 首帧稳定后显示 Chrome；无登录常驻进程。\n");
    if (result.compileCachePrepared) process.stdout.write("DSH 启动缓存：已在 create 阶段准备\n");
    if (result.loadingRenderer === "chrome-html") process.stdout.write("启动画面：由 Chrome 渲染三平台统一的小鲸鱼 loading\n");
    if (result.usesLoadingScreen) process.stdout.write("页面交接：DSH 完整就绪后在同一 Chrome 窗口淡出 loading\n");
