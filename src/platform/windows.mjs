...
export async function createWindowsLauncher(config, chrome, env = process.env) {
  ...
  try {
    ...
    const storedConfig = {
      ...
      hostBrowserTimingPath: path.join(stateDirectory, "browser-startup.log"),
    ...
    await writeText(nativeLauncherSourcePath, renderWindowsNativeLauncherSource({
...
function compileWindowsLauncher({ powerShellPath, sourcePath, outputPath, outputType = "WindowsApplication" }) {
  ...
  if (result.error) throw new Error(`无法编译 Windows 原生启动器：${result.error.message}`);
  if (result.status !== 0) throw new Error(`无法编译 Windows 原生启动器：${(result.stderr || result.stdout).trim()}`);
}
